...
def start_attack(ip, port, duration, intensity=1000000):
    ...
    if attack_process is None or attack_process.poll() is not None:
        ...
        try:
            ...
            logger.info(f'Attack started on {ip}:{port} for {duration} minutes with intensity {intensity}.')
            # Start a thread to monitor the attack process
...
def monitor_attack(process, duration):
    ...
    if process.poll() is None:
        process.terminate()
        process = None
        logger.info('Attack process terminated by monitor.')
...
def stop_attack():
    ...
    if attack_process and attack_process.poll() is None:
        ...
        logger.warning('No attack is currently running.')
        return False
